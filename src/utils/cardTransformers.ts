...
export async function transformBeastToCardData(beast: CollectionEntry<"bestiary">): Promise<CardData> {
    const { data } = beast;
    ...
    return {
        ...
        stats: [
            ...
            {
                label: "Скорость",
                value: await Promise.all(data.speeds.map(async speed => 
        ...
        sections: await Promise.all([
            {
                title: "Сопротивления",
                items: data.resistances ? await Promise.all(data.resistances.map(res => getEntry(res).then(entry => entry.data.title))) : []
            },
            {
                title: "Иммунитеты",
                items: data.immunities ? await Promise.all(data.immunities.map(imm => getEntry(imm).then(entry => entry.data.title))) : []
            },
            {
                title: "Уязвимости",
                items: data.vulnerabilities ? await Promise.all(data.vulnerabilities.map(vul => getEntry(vul).then(entry => entry.data.title))) : []
            }
        ].filter(section => section.items.length > 0))
    };
}

export async function transformSpellToCardData(spell: CollectionEntry<"spells">): Promise<CardData> {
    const { data } = spell;
    ...
    return {
        ...
        sections: await Promise.all([
            {
                title: "Компоненты",
                items: data.components ? await Promise.all(data.components.map(res => getEntry(res).then(entry => entry.data.title))) : []
            }
        ].filter(section => section.items.length > 0))
    };
}
